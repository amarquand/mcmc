function [G InvG CholG GDeriv] = RMHMC_compute_G_f(f,InvK, T)
    ...
    if nargout > 2                % do we need the Cholesky decomposition?
        ...
        if nargout > 3
            ...
            for d = 1:D
                ...
                for c = 1:n_gps
                    if c == class
                        dP(sample,c) = P(sample,class).*(1-P(sample,class));%0.01;
                    else
                        dP(sample,c) = -P(sample,class).*P(sample,c);%0.001;
                    end
                end
