...
for g = 1:opt.nGibbsIter
    ...
    %%%%%%%%%% 
    gxargs_f = {InvK, Y};
    fxargs_f = {Y, InvK, LogDetK, opt.PoissScale};
    if opt.UseRMHMC
        ...
        L_Gf        = chol(Gf)';
        InvGf       = inv(Gf);
        [Ef, f_new] = hmc(f, 'hmc_poiss_posterior_f', opt.rmhmc,  L_Gf, InvGf, fxargs_f{:});           
    ...
    if opt.OptimiseTheta
        ...
        [tmp, LogLik_f]     = likelihood_poisson(f,Y,opt.PoissScale);
        [tmp, LogLik_f_new] = likelihood_poisson(f_new,Y,opt.PoissScale);
